...
router.get('/', async (req, res) => {
    const recipes = await db.query('SELECT a.recipeName, a.instructions, b.ingredientName FROM recipe a INNER JOIN IngredientInRecipe c ON a.id = c.recipeId INNER JOIN ingredient b ON b.id = c.ingredientId;'); // await tähendab et ootab kuni saab päringult vastuse ja alles siis liigub edasi
    const recipeMap = {};
...
router.get('/recipeingredients', async (req,res) => {
    const recipes = await db.query('SELECT a.recipeName, b.ingredientName FROM recipe a INNER JOIN IngredientInRecipe c ON a.id = c.recipeId INNER JOIN ingredient b ON b.id = c.ingredientId;'); // await tähendab et ootab kuni saab päringult vastuse ja alles siis liigub edasi
